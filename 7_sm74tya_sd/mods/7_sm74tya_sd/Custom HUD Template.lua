...
local function on_hud_render() -- Handles the HUD layouts 
    ...
    if custom_hud then -- HUD layout
        ...
        render_coins_segment(screenWidth - 76, 15, 1, 1) -- Coins Segment
        render_stars_segment(22, 15, 1, 1) -- Stars Segment
        render_power_meter(halfScreenWidth - 51, 9, 65, 65) --scale 65 for default power meter
        --render_cap_timer(math.ceil(gMarioStates[0].capTimer/30), 22, screenHeight - 35) -- Cap Timer Featured in Some Rom Hacks
        if newTimer then
            render_timer(hud_get_value(HUD_DISPLAY_TIMER), screenWidth - 120, screenHeight - 200) --New Timer
        else
