...
---@return nil
local function render_element(render, x, y, scale, align_bottom, should_append_zero)
    ---@type MarioState
    ...
    if render_texture then
        ...
        render_custom_text_element("x", x + 16, y, scale, false, align_bottom)
...
local function render_timer(timer_value, x, y)
    ...
    local minutes = (tostring(math.floor((time / 30) // 30))) -- Strange but it does work
    local seconds = append_zero(tostring(time // 30))
    local milliseconds = append_zero(tostring((time % 30) * 3)) -- Decimal expansion
    render_custom_text_element(minutes, x + 11, y, 1, false, false)
...
local function hud_render()
    ...
    local x = 24
    local y = 15
    render_element(HUD_RENDER_LIVES, x, y, 1, false, true)
    -- Stars
    y = 15
    x = screen_width - 95
    ...
    if gNetworkPlayers[0].currLevelNum == LEVEL_CASTLE_GROUNDS or gNetworkPlayers[0].currLevelNum == LEVEL_CASTLE_COURTYARD or gNetworkPlayers[0].currLevelNum == LEVEL_CASTLE then
        render_element(HUD_RENDER_COINS, 99999, 99999, 1, false, false)
    else
    ...
    if PowerState then
        ...
        elseif Py > 5 then
            ...
            PowerTimer = 45 --restart the timer if PowerState is true
        end
    else
        if PowerTimer > 0 then
            PowerTimer = PowerTimer - 1 --counting
        elseif Py > 5 -100 then
    ...
    --djui_chat_message_create(tostring(Py)) --for testing
    -- Caps
    ...
    if hud_get_value(HUD_DISPLAY_FLAGS) & HUD_DISPLAY_FLAGS_TIMER ~= 0 then
        ...
        x = screen_width * 0.5 - 61.3 -- It took trial and error to find this value
