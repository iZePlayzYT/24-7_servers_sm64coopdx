...
function bhv_pswitch_loop(obj)
    ...
    switch (obj.oAction, {
        ...
        [PURPLE_SWITCH_PRESSED] = function ()
            cur_obj_scale_over_time(2, 10, 1.0, 0.2) -- this is different but idk the values
            if obj.oTimer == 10 then --both 2nd param of cur_obj_scale_over_time and this needs to be the same number
                --cur_obj_play_sound_2(SOUND_GENERAL2_PURPLE_SWITCH) not accurate
                obj.oAction = PURPLE_SWITCH_TICKING
                --cur_obj_shake_screen(SHAKE_POS_SMALL) not accurate
				Swap_Coins_Box()
			end
        end,
        --[[
        ...
        [PURPLE_SWITCH_TICKING] = function ()
			...
			--	play_sound(SOUND_GENERAL2_SWITCH_TICK_FAST, sound_source) it always plays the TICK_SLOW sound
			--else
			play_sound(SOUND_GENERAL2_SWITCH_TICK_SLOW, sound_source)
            --end
			if obj.oTimer > 200 then -- still not sure about the time
        ...
        [PURPLE_SWITCH_UNPRESSED] = function ()
            cur_obj_scale_over_time(2, 10, 0.2, 1.0)
